...
Add dummy string to Message for size
```
    // client.go
...
No errors and continue to run like using tcp socket connection.

When using tcp connection, all codes are not different besides the address in server.go and client.go. They are also at https://github.com/dizecto/grpc-go-uds-test.git.

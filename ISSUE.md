...
github.com> cd grpc-go\examples
github.com\grpc-go\examples> go mod vendor
github.com\grpc-go\examples> go clean -modcache
github.com\grpc-go\examples> go clean -cache
github.com\grpc-go\examples> go clean -testcache
github.com\grpc-go\examples> go build -mod vendor route_guide\server\server.go
github.com\grpc-go\examples> go build -mod vendor route_guide\client\client.go
```

##### Execute 'server.exe' and 'client.exe' in separate command windows.
1. Create a "temp" directory in the D drive
```
github.com\grpc-go\examples> mkdir D:\temp
```
2. Launch 'server.exe'
```
